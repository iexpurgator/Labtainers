...
def main():
    ...
    if numprocessor == 1:
        print(
            "Labtainers will perform better with two processors allocated to the Linux host."
        )
    if memoryinGB < 1.8:
        print(
            "Labtainers may perform better with at least 2 GB of RAM allocated to the Linux host."
        )
    if numprocessor == 1 or memoryinGB < 1.8:
        user_input = None
        user_input = input(
            "Would like to shutdown the host so you can allocate more resources? (yes/no)\n"
        )
