...
if [ "$result" == "0" ] && [ $force_build = "False" ]; then
    ...
    ORIG_PWD=`pwd`
    echo $ORIG_PWD
    ../labtainer_student/bin/checkTars.py $LAB_DIR $imagename
    ...
    cp ../labtainer_student/lab_bin/ParameterParser.py $TMP_DIR/.local/bin/
    cp ../labtainer_student/lab_bin/ParameterizeLogging.py $TMP_DIR/.local/bin/
