        ...
        for member_info in zip_file.infolist():
            ...
            if 'assignsubmission_file' in member:
                ...
                if '_at_' not in student:
                    ...
                    for zf in student_fu.infolist():
                        if zf.filename.endswith('.zip') or zf.filename.endswith(
                                '.lab'):
            ...
            if filename.endswith('.zip') or filename.endswith('.lab'):
                ...
                if lab == expect_lab:
                    ...
                    print(
                        'Those results are copied into the %s xfer directory.  Consider regrading that lab with nww results.'
                        % lab)
                lab_xfer = os.path.join(xfer, lab)
                ...
                if not os.path.isdir(lab_xfer):
                    print(
                        'ERROR ******: no such xfer directory %s. student: %s, file %s'
                        % (lab_xfer,
                           student,
                           member))
                    continue
                        ...
                        if zi == 'docs.zip':
                            ...
                            with zipfile.ZipFile(docs_zip_data) as zip_docs:
                                for zdoc_info in zip_docs.infolist():
                                    ...
                                    doc_date_time = time.mktime(
                                        zdoc_info.date_time + (0,
                                                               0,
                                                               -1))
                                    #print('student %s look for report in %s' % (student, zdoc))
                                    fname, ext = os.path.splitext(zdoc)
                                    if (ext == '.docx' or ext == '.odt' or
                                            ext == '.xlsx') and (
                                                fname + '.pdf'
                                                not in zip_docs.namelist()):
                                        if not os.path.isfile(
                                            ...
                                            print(
                                                'found doc at %s, do not overwrite'
                                                %
                                                os.path.join(target_dir,
                                                             zdoc))
                ...
                if (ext == '.docx' or ext == '.odt' or ext == '.xlsx'):
                    ...
                    shutil.copyfileobj(source, target)
                    os.utime(os.path.join(target_dir,
                                          filename),
                             (date_time,
                              date_time))
