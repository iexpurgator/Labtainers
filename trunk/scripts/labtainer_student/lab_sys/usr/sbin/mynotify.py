    ...
    for line in fh:
        if not line.strip().startswith('#'):
            ...
            try:
                ...
                logger.debug('could not add watch for %s %s' % (watch.path,
                                                                watch.flag))
#
...
while True:
    for event in inotify.read():
        ...
        if cmd is not None:
            ...
            child = subprocess.Popen(sys_cmd,
                                     shell=True,
        ...
        if not os.path.isfile(notifyoutfile_ts):
            ...
            if not is_a_file:
                ...
                logger.debug('No file named: %s, look previous second for %s' %
                             (notifyoutfile_ts,
                              tmpfile))
        ...
        if is_a_file:
            ...
            if notify_cb is not None:
                sys_cmd = '%s %s %s %s %s >> %s 2>/dev/null' % (
                    notify_cb,
                    watch.path,
                    watch.flag,
                    cmd_user,
                ...
                with open(notifyoutfile_ts, 'a') as fh:
                    fh.write('path: %s cmd: %s user: %s' % (watch.path,
                                                            cmd,
                                                            cmd_user))
        else:
            if notify_cb is not None:
                ''' only write to file if notify_cb generates output '''
                sys_cmd = '%s %s %s %s "%s"' % (notify_cb,
                                                watch.path,
                                                watch.flag,
                                                cmd_user,
                                                cmd)
