...
top level scripts share functions found in scripts/labtainer_student/{\bf bin}/labutils.py.  The 
...
\item Regression testing of grading functions is performed by labtainer_instructor/regress.py.
...
Student scripts, e.g., {\tt labtainer}, run from the scripts/labtainer_student directory.
...
\subsection{mynotify}
The mynotify runs as a service.  It is installed from the labtainer_student/lab\_bin directory.
...
labtainer_student/.tmp/<lab>/.  This ensures that subsequent runs of the lab always
...
The Flask web server is integrated with the grader container.  The {\tt labtainer_instructor/flask/server.py} program implements
